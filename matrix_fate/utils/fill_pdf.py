...
# # class GenerateMatrixPDFView(APIView):
# #     def post(self, request):
# #         try:
# #             pdf_path = fill_matrix_pdf(request.data)

# #             return FileResponse(
# #                 open(pdf_path, "rb"), filename="matrix_result.pdf", as_attachment=True
# #             )
...
# def insert_centered_text(page, text, center_x, y, fontname, font_obj, fontsize=12, color=(0, 0, 1)):
#     text = str(text).strip()
#     if not text:
#         return
...
#         print(f"Inserting '{text}' at ({x}, {y}), size={fontsize}, key={key}")

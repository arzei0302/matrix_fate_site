...
# from matrix_fate.common.permissions import IsActivePaidUser, is_active_paid_user
...
class CategoryWithSpiritualKarmaAPIView(APIView):
    ...
    def get(self, request, category_id_or_title):
        ...
        # 2. Проверяем, платный ли пользователь
        if not is_active_paid_user(request.user):
            # Если нет подписки или нет авторизации — возвращаем только id + title
            return Response({
                "category": {
                    "id": category.id,
                    "title": category.title,
        ...
        # 3. Пользователь платный → запрашиваем задачи

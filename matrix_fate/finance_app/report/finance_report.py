...
class FinancePDFInputSerializer(serializers.Serializer):
...
class FullFinancePDFView(APIView):
    permission_classes = [IsAuthenticated]

    @extend_schema(
        summary="Скачать PDF 'Финансы",
        description="PDF-документ на основе данных истории расчёта.(категория только finance)",
        request=FinancePDFInputSerializer,
    ...
    def post(self, request):
        try:
            ...
            category = request.data.get("category", "finance")
            ...
            return FileResponse(pdf_buffer, filename="finance_report.pdf", as_attachment=True)

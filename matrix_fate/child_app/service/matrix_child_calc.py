...
from child_app.serializers.matrix_child_program_serializers import (
    MatrixChildInputSerializer, MatrixChildOutputSerializer, MatrixChildProgramSerializer)
from finance_app.service.service import get_matching_programs
...
@extend_schema(
    tags=['Child Matrix'],
    request=MatrixChildInputSerializer,
    responses={
        200: MatrixChildOutputSerializer,
        400: OpenApiResponse(
...
@api_view(["POST"])
def calculate_child_matrix_view(request):
    """Категории: ('matrix_fate', 'finance', 'compatibility', 'child')"""
    serializer = MatrixChildInputSerializer(data=request.data)
    ...
    output_serializer = MatrixChildOutputSerializer(data=matrix_values)

...
class GuestFinancePDFInputSerializer(serializers.Serializer):
...
class GuestFinancePDFView(APIView):
    permission_classes = [AllowAny]
    @extend_schema(
    summary="Скачать гостевой PDF 'Финансы",
    description="Генерирует PDF-схему матрицы на основе введённых даты/месяца/года без сохранения в базу данных.",
    request=GuestFinancePDFInputSerializer,
    tags=["Matrix Fate Reports"],
    responses={
            200: OpenApiResponse(
                response=OpenApiTypes.BINARY,
                description='PDF-файл c финансовой матрицей'
            ),
            400: OpenApiResponse(
    ...
    def post(self, request):
        serializer = GuestFinancePDFInputSerializer(data=request.data)
        ...
        # можно добавить остальные маркеры по аналогии
        ...
        with open(visual_pdf_path, "rb") as f:
            result_buffer = io.BytesIO(f.read())
            result_buffer.seek(0)

        return FileResponse(result_buffer, filename="guest_matrix_scheme.pdf", as_attachment=True)
